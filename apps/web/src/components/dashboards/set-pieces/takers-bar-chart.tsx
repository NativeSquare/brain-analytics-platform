...
interface TakersBarChartProps {
  items: SetPiece[];
  onTakerClick: (taker: string | null) => void;
  highlightedPlayer: string | null;
}

const TakersBarChart = ({
  items,
  onTakerClick,
  highlightedPlayer,
}: TakersBarChartProps) => {
  const data = useMemo(() => {
    ...
    for (const sp of items) {
      const taker = sp.taker ?? "Unknown";
      counts.set(taker, (counts.get(taker) ?? 0) + 1);
    }
  ...
  return (
    <div className="rounded-xl border bg-card p-4 shadow-sm">
      <div className="mb-3 text-sm font-semibold uppercase">Top Takers</div>
        ...
        {data.map(([label, count]) => {
          ...
          return (
            <button
              ...
              onClick={() => onTakerClick(isActive ? null : label)}
            >
              <span
              ...
              <div className="relative flex-1">
                <div
                  className={`h-5 rounded ${isActive ? "bg-primary" : "bg-primary/70"}`}
                  style={{
                    width:
