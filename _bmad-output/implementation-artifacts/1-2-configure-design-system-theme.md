...
4. **Dark/light mode toggle is functional** — The `next-themes` `ThemeProvider` is integrated into the root layout (`apps/admin/src/app/layout.tsx`) wrapping the application, enabling `class`-based dark mode switching. The `<html>` element receives the `suppressHydrationWarning` attribute. A basic theme toggle component exists for switching between light and dark modes.
...
- [x] **Task 1: Run the shadcn preset initialization** (AC: #1, #2, #3)
  - [x] 1.1: Run `npx shadcn@latest init --preset b7lRK5amaQ` from `apps/admin/`
...
- [x] **Task 2: Integrate ThemeProvider for dark/light mode** (AC: #4)
  ...
  - [x] 2.2: Update `apps/admin/src/app/layout.tsx` to wrap the app with `ThemeProvider` from `next-themes` with `attribute="class"`, `defaultTheme="system"`, `enableSystem`, and `disableTransitionOnChange`
  - [x] 2.3: Add `suppressHydrationWarning` to the `<html>` element to prevent hydration mismatch warnings from `next-themes`
  - [x] 2.4: Create a `ThemeToggle` component at `apps/admin/src/components/shared/ThemeToggle.tsx` using shadcn/ui `Button` and `DropdownMenu` components with sun/moon icons, offering Light / Dark / System options via `useTheme()` hook

- [x] **Task 3: Update app metadata and branding** (AC: #6)
  - [x] 3.1: Update the `metadata` export in `apps/admin/src/app/layout.tsx` — set `title` to `"BrainAnalytics"` and `description` to `"Football Operations Platform"`

- [x] **Task 4: Visual verification of themed components** (AC: #5)
  - [x] 4.1: Verify the existing 56 shadcn/ui components in `apps/admin/src/components/ui/` use CSS variable references (e.g., `bg-primary`, `text-foreground`) and not hardcoded color values
  ...
  - [x] 4.3: Confirm the sidebar component (`apps/admin/src/components/ui/sidebar.tsx`) uses the sidebar-specific CSS variables (`--sidebar`, `--sidebar-foreground`, etc.) and renders correctly with the new theme
...
```tsx
// apps/admin/src/components/shared/ThemeToggle.tsx
"use client"
...
- **Manual verification checklist:**
  - `npx shadcn@latest init --preset b7lRK5amaQ` runs successfully from `apps/admin/`
...
|------|-------------|-------------|
| `apps/admin/src/app/globals.css` | Modified | CSS variables updated by preset (colors, radius, possibly fonts) |
| `apps/admin/components.json` | Modified | Preset configuration applied (style, baseColor, iconLibrary) |
| `apps/admin/src/app/layout.tsx` | Modified | Add ThemeProvider, suppressHydrationWarning, update metadata |
| `apps/admin/src/components/shared/ThemeToggle.tsx` | Created | New theme toggle component |
...
- `ThemeToggle` component created at `apps/admin/src/components/shared/ThemeToggle.tsx` using `@tabler/icons-react` sun/moon icons.
...
- `apps/admin/src/app/globals.css` — Modified (CSS variables updated by preset)
- `apps/admin/components.json` — Modified (style, baseColor, iconLibrary updated)
- `apps/admin/src/app/layout.tsx` — Modified (ThemeProvider, suppressHydrationWarning, metadata, fonts)
- `apps/admin/src/components/shared/ThemeToggle.tsx` — Created (theme toggle component)
- `apps/admin/src/components/custom/password-input.tsx` — Modified (lucide→tabler icons, lint fix)
- `apps/admin/src/components/ui/*.tsx` — Modified (53 files re-installed by preset with tabler icons)
- `apps/admin/src/lib/utils.ts` — Modified (updated by preset)
- `apps/admin/package.json` — Modified (shadcn dependency added by preset)
