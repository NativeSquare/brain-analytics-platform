...
- [ ] Task 1: Create `components/shared/` directory structure (AC: #6)
  - [ ] 1.1 Create `apps/admin/src/components/shared/` directory
  - [ ] 1.2 Create barrel `index.ts` export file

- [ ] Task 2: Build EventTypeBadge component (AC: #1)
  - [ ] 2.1 Define `EventType` type: `"match" | "training" | "meeting" | "rehab"`
  - [ ] 2.2 Define color mapping using CVA variants extending the base `Badge` component
  - [ ] 2.3 Map each event type to a Lucide icon (e.g., `Trophy` for match, `Dumbbell` for training, `Users` for meeting, `Heart` for rehab)
  - [ ] 2.4 Implement `size` prop ("sm" | "default") controlling padding/font-size
  - [ ] 2.5 Export `EventTypeBadge` from `EventTypeBadge.tsx`

- [ ] Task 3: Build StatusBadge component (AC: #2)
  - [ ] 3.1 Define `PlayerStatus` type: `"active" | "on-loan" | "left-the-club"`
  - [ ] 3.2 Define color mapping using CVA variants extending the base `Badge` component
  - [ ] 3.3 Map each status to a Lucide icon (e.g., `CircleCheck` for active, `ArrowRightLeft` for on-loan, `LogOut` for left-the-club)
  - [ ] 3.4 Export `StatusBadge` from `StatusBadge.tsx`

- [ ] Task 4: Build ReadTrackingIndicator component (AC: #3)
  - [ ] 4.1 Define props: `opened: number`, `total: number`, `compact?: boolean`
  - [ ] 4.2 Compose with shadcn/ui `Progress` component for visual bar
  - [ ] 4.3 Calculate and display percentage; handle `total === 0` edge case
  - [ ] 4.4 Implement compact mode (inline text + thin bar) vs. default mode (stacked text + bar)
  - [ ] 4.5 Export `ReadTrackingIndicator` from `ReadTrackingIndicator.tsx`

- [ ] Task 5: Build FolderBreadcrumb component (AC: #4)
  - [ ] 5.1 Define `BreadcrumbSegment` type: `{ id: string; label: string }`
  - [ ] 5.2 Define props: `segments: BreadcrumbSegment[]`, `onNavigate: (folderId: string) => void`
  - [ ] 5.3 Compose with shadcn/ui Breadcrumb primitives (Breadcrumb, BreadcrumbList, BreadcrumbItem, BreadcrumbLink, BreadcrumbPage, BreadcrumbSeparator)
  - [ ] 5.4 Implement ellipsis truncation when segments.length > 3 (show first, ellipsis, last two) using BreadcrumbEllipsis
  - [ ] 5.5 Export `FolderBreadcrumb` from `FolderBreadcrumb.tsx`

- [ ] Task 6: Build NotificationCenter component (AC: #5)
  - [ ] 6.1 Define `NotificationItem` type: `{ id: string; title: string; message: string; createdAt: number; read: boolean }`
  - [ ] 6.2 Define props: `notifications: NotificationItem[]`, `unreadCount: number`, `onMarkAllRead: () => void`, `onNotificationClick?: (id: string) => void`
  - [ ] 6.3 Build bell icon button with unread count badge overlay (absolute positioned badge on the Bell icon)
  - [ ] 6.4 Build popover/dropdown with notification list using shadcn/ui `Popover` (PopoverTrigger + PopoverContent)
  - [ ] 6.5 Implement notification item row: icon, title, message truncated, relative timestamp (via date-fns `formatDistanceToNow`)
  - [ ] 6.6 Add "Mark all as read" button in popover header
  - [ ] 6.7 Add empty state when no notifications
  - [ ] 6.8 Export `NotificationCenter` from `NotificationCenter.tsx`

- [ ] Task 7: Barrel export & type exports (AC: #6)
  - [ ] 7.1 Update `index.ts` to export all components and their prop types
  - [ ] 7.2 Verify all components render independently (no Convex imports, all data via props)
